...
fun main() {
    ...
    val output1 = towerOfHanoi(input1)
    val output2 = towerOfHanoi(input2)

    println("Output 1: ${output1.toJson()}")
    println("Output 2: ${output2.toJson()}")
}


fun towerOfHanoi(A: Int): Array<IntArray> {
    val result = ArrayList<IntArray>()
    moveDisks(A, 1, 2, 3, result)
    return result.toTypedArray()
}

fun moveDisks(n: Int, source: Int, auxiliary: Int, destination: Int, result: ArrayList<IntArray>) {
    if (n == 1) {
        val move = intArrayOf(n, source, destination)
        result.add(move)
        return
    }

    moveDisks(n - 1, source, destination, auxiliary, result)

    val move = intArrayOf(n, source, destination)
    result.add(move)

    moveDisks(n - 1, auxiliary, source, destination, result)
}
